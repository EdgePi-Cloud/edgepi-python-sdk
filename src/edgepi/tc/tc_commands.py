...
def _negative_temp_check(temp_code, temp_code_high_byte, temp_bits):
...
def code_to_temp(code_bytes:list):
    ...
    cj_code = _negative_temp_check(cj_code, cj_high_byte, TempBits.CJ_BITS.value)

    # combine and arrange the temperature bits in order from msb to lsb and eliminate empty placeholder bits
    lt_code = ((lt_high_byte<<16) + (lt_mid_byte<<8) + lt_low_byte) >> 5
    lt_code = _negative_temp_check(lt_code, lt_high_byte, TempBits.LT_BITS.value)
