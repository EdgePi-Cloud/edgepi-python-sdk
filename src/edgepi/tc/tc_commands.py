...
from edgepi.tc.tc_constants import TempBits, Masks
...
class TempCode():
    ''' Holds data needed to update a MAX31856 temperature threshold configuration, where
        ...
        Attributes:
            ...
            dec_val (int): the decimal value to be written to this setting, represented
                            as a binary value (uint)
            
            num_int_bits: the number of bits assigned to integer values for this setting

            num_dec_bits: the number of bits assigned to decimal values for this setting

            num_int_bits: the number of bits assigned to filler values for this setting

            start_addx: address of the MSB for this setting
    '''
    int_val : int
    dec_val : int
    num_int_bits : int
