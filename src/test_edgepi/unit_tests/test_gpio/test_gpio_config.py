...
@pytest.mark.parametrize(
    "name, setoutput, clearoutput, address, result",
    [
        (
            "AO_EN1",
            ...
            ["AO_EN1", OpCode(1, 2, 0xFE), OpCode(0, 2, 0xFE), 32],
        ),
        (
            "AO_EN1",
            GpioAOutputSet.SET_OUTPUT_2.value,
            GpioAOutputClear.CLEAR_OUTPUT_2.value,
            GpioExpanderAddress.EXP_ONE.value,
            ["AO_EN1", OpCode(2, 2, 0xFD), OpCode(0, 2, 0xFD), 32],
        ),
        (
            "AO_EN1",
            GpioAOutputSet.SET_OUTPUT_3.value,
            GpioAOutputClear.CLEAR_OUTPUT_3.value,
            GpioExpanderAddress.EXP_ONE.value,
            ["AO_EN1", OpCode(4, 2, 0xFB), OpCode(0, 2, 0xFB), 32],
        ),
        (
            "AO_EN1",
            GpioAOutputSet.SET_OUTPUT_4.value,
            GpioAOutputClear.CLEAR_OUTPUT_4.value,
            GpioExpanderAddress.EXP_TWO.value,
            ["AO_EN1", OpCode(8, 2, 0xF7), OpCode(0, 2, 0xF7), 33],
        ),
    ],
)
...
def test_generate_pin_info_dac(
    config="dac",
    pin_name_list=[
        "AO_EN1",
        "AO_EN4",
    ...
    for pin, outputset, outputclear, pin_name in zip(
        pin_list[:-1], GpioAOutputSet, GpioAOutputClear, pin_name_list
    ):
...
def test_generate_pin_info_led(
    config="led",
    pin_name_list=[
        "LED_OVR1",
    ...
    pin_list = generate_pin_info(config)
    for pin, outputset, outputclear, pin_name in zip(
        pin_list, GpioBOutputSet, GpioBOutputClear, pin_name_list
    ):
        assert pin.name == pin_name
        assert pin.set_code == outputset.value
        assert pin.clear_code == outputclear.value
