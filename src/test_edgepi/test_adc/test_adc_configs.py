"""unit tests for adc_configs module"""
...
@pytest.mark.parametrize(
    "dev_id, rev_id, error", [(1, 1, FrozenInstanceError), (0, 1, FrozenInstanceError)]
)
...
@pytest.mark.parametrize(
    "reset, vbias, intref, result", [(1, 0, 1, 0x11), (0, 0, 1, 0x01)]
)
...
@pytest.mark.parametrize(
    "timeout, status, crc, result", [(0, 1, 1, 0x5), (0, 0, 3, 0x03), (1, 1, 3, 0x0F)]
)
...
@pytest.mark.parametrize(
    "refrev, runmode, chop, delay, result",
    [(0, 0, 0, 0, 0x0), (1, 1, 1, 1, 0xD1), (0, 0, 3, 0, 0x30)],
)
...
@pytest.mark.parametrize(
    "adc_filter, sbadc, sbpol, sbmag, result",
    [(4, 0, 0, 0, 0x80), (4, 1, 1, 7, 0x9F), (7, 0, 0, 7, 0xE7)],
)
def test_mode1_register_value(adc_filter, sbadc, sbpol, sbmag, result, adc_configs):
    adc_configs[regs.REG_MODE1.value].filter = adc_filter
...
@pytest.mark.parametrize(
    "bypass, gain, dr, result", [(0, 0, 4, 0x04), (1, 7, 15, 0xFF), (1, 1, 3, 0x93)]
)
