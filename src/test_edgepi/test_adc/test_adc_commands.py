...
@pytest.mark.parametrize("address, num_of_regs, error", [(1, 3.2, ValueError),
                                                          (3.2, 1, ValueError)])
...
Out: List of bytes Op-code and dummy bytes to transfer
"""
@pytest.mark.parametrize("address, values, result", [(1, [2], [0x41, 0, 2]),
                                                     ...
                                                     (16, [25, 55, 5, 2, 5, 25, 55, 55, 25, 2], [0x50, 9, 25, 55, 5, 2, 5, 25, 55, 55, 25, 2])])
def test_write_register_command(address, values, result, adc_ops):
    assert adc_ops.write_register_command(address, values) == result

@pytest.mark.parametrize("address, values, error", [(1, [-1, 2], ValueError),
                                                     (2, [23, 25, 256, 102], ValueError)])
